...
class HousingCrawler(object):
    def __init__(self,
        ...
        if filters is None:
            assert not None in [site, n_beds, price_min, price_max,
                                area_min], 'Please make sure either all args are filled or filters are provided'
    ...
    def format_data(self):
        ...
        l_COLUMNS = ['id', 'url', 'datetime', 'created', 'last_updated',
                     'geotag', 'price', 'area', 'bedrooms', 'bathrooms']
    ...
    def enrich_traveldata(self, destination=None, mode=None):
        ...
        :params destination: dict with keys (lat, lng) of destination or dict with keys refering
        to names of destinations and values dicts with (lat, lng) as keys
        ...
        assert isinstance(destination, dict), "destination is either must be dict, {} given".format(
            type(destination))
        ...
        for dest_name, dest_lat_lng in d_destinations.items():
            for mode_ in l_modes:
                ...
                dist_dur_colname = "_".join(
                    ["dist_dur", str(dest_name), str(mode_)])
                ...
                self.df_res[dist_dur_colname] = self.df_res.geotag_rounded.apply(
                    lambda x: get_travel_info(
                        x, destination=dest_lat_lng, mode=mode_)
                )

                # distance column
                dist_colname = "_".join(
                    ["distance", str(dest_name), str(mode_)])
                self.df_res[dist_colname] = self.df_res[dist_dur_colname].apply(
                    lambda x: x[0])

                # duration column
                dur_colname = "_".join(
                    ["duration", str(dest_name), str(mode_)])
                self.df_res[dur_colname] = self.df_res[dist_dur_colname].apply(
                    lambda x: x[1])
    ...
    def get_travel_score(self):
        ...
        assert isinstance(
            self.d_destinations, dict), "You must run enrich_traveldata to get travel data"
        assert isinstance(
            self.l_modes, list), "You must run enrich_traveldata to get travel data"
        ...
        for dest_name in self.d_destinations.keys():
            ...
            # list of all
            l_mode_scores = ['_'.join([mode_, dest_name, "score"])
                             for mode_ in self.l_modes]
            # the travel score is the max of all mode scores
            self.df_res[travel_score_name] = self.df_res[l_mode_scores].max(
                axis=1)
    ...
    def get_aggregate_score(self):
        ...
        l_travel_scores = ['_'.join(["travel", dest_name, "score"])
                           for dest_name in self.d_destinations.keys()]
        l_all_scores = l_adhoc_scores + l_travel_scores
        self.df_res['score'] = self.df_res[l_all_scores].apply(
            geometric_mean, axis=1)
    ...
    def _pull_data_fromcraig(filters, site, area, day=datetime.date.today(), limit=None):
        ...
        res_gen = cl_h.get_results(
            limit=limit, include_details=True, geotagged=True)
        res = list(res_gen)
        return res
    ...
    def get_column_score(self, val, val_name):
        ...
        norm_val = self.normalize_val(
            val, min_bound=min_val, max_bound=max_val)
        return norm_val
